...
void playapi_cli_base::perform_auth() {
    ...
    if (opt_access_token.length() > 0) {
        // BedrockHarbor: one-shot login with a Google embedded-setup access token
        // (oauth2_4/...). On success the long-lived master token is persisted via
        // --save-auth so later runs need no token at all.
        try {
